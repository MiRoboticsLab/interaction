...
{
std::mutex touch_data_cvm_;                       /*!< Touch 数据 cvm */
std::condition_variable touch_data_cv_;           /*!< Touch 数据 cv */

bool Touch::SetData(const toml::value &)
{
  try {
...
bool Touch::SetMechanism(const toml::value & _params_toml)
{
  try {
    Debug("%s", std::string(__FUNCTION__).c_str());
    this->sub_cb_group_ =
    ...
    this->sub_ptr_ = this->node_mortal_ptr_->create_subscription<MsgTouchStatus>(
      toml::find_or(
        _params_toml, "vp", "init", "topic", "touch", "touch_status"),
      SubscriptionSensorQos,
      std::bind(&Touch::SubCB, this, std::placeholders::_1),
      sub_option);
  } catch (const std::exception & e) {
...
void Touch::SubCB(const MsgTouchStatus::SharedPtr _msg_ptr)
{
  {
...
MsgTouchStatus Touch::GetData(const int _timeout)
{
  ...
  try {
    ...
    std::unique_lock<std::mutex> lk(touch_data_cvm_);
    touch_data_cv_.wait_for(
